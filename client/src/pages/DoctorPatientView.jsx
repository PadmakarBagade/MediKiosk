...
const DoctorPatientView = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (res.success) {
          ...
          setSummaryText(res.consultation.aiSummary?.doctorFacingText || '');
  ...
  const handleSaveSummary = async () => {
    ...
    try {
      const res = await updateAiSummary(id, { doctorFacingText: summaryText });
      if (res.success) {
        setConsultation(res.consultation);
        setSummaryText(res.consultation.aiSummary?.doctorFacingText || '');
        setIsEditingSummary(false);
        setSummarySaveSuccess(true);
        setTimeout(() => setSummarySaveSuccess(false), 4000);
      }
    } catch (e) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          <div className="bg-white rounded-3xl border-2 border-slate-200 shadow-sm p-6 sm:p-8 space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                {!isEditingSummary && (
                  <button
                    type="button"
                    onClick={() => {
                      setSummaryText(consultation.aiSummary?.doctorFacingText || '');
                      setIsEditingSummary(true);
                    }}
                    className="px-3 py-1.5 rounded-xl bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 text-xs font-bold transition flex items-center gap-1.5"
                  >
            ...
            {summarySaveSuccess && (
              <div className="p-3.5 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl flex items-center gap-2 text-xs font-semibold animate-fadeIn">
                <CheckCircle2 className="w-4 h-4 text-emerald-600 flex-shrink-0" />
                <span>AI summary text updated and recorded with doctor_verified provenance!</span>
              </div>
            )}

            {/* Editable or Static Structured Summary */}
            {isEditingSummary ? (
              <div className="space-y-3">
                <div className="flex items-center justify-between text-xs text-slate-500">
                  <span className="font-semibold text-emerald-800 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-200">
                    Editing Clinician-Facing Summary Directly
                  </span>
                  <span className="text-[11px] text-slate-400">
                    Saves to consultation & tags provenance as doctor_verified
                  </span>
                </div>
                ...
                <textarea
                  ...
                  className="w-full font-mono text-xs sm:text-sm p-4 rounded-2xl border-2 border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none leading-relaxed bg-white text-slate-900 resize-y shadow-inner"
                  placeholder="Edit clinical summary directly..."
                />
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-4 py-2 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-100 font-bold text-xs transition"
                    >
                      Cancel
                    </button>
                    <button
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between pt-1">
                  <div className="flex items-center gap-2">
                    {consultation.sourceProvenance?.some(
                      (p) => p.field === 'aiSummary.doctorFacingText' && p.sourceType === 'doctor_verified'
                    ) ? (
                      <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-700 bg-emerald-100/70 border border-emerald-200 px-2.5 py-1 rounded-full">
                        <CheckCircle2 className="w-3.5 h-3.5" />
                        Doctor Verified & Edited
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 text-[11px] text-slate-400">
                        Source: AI Generated Synthesis
                      </span>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    className="text-xs font-bold text-emerald-700 hover:text-emerald-800 flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 transition"
                  >
                    <Edit3 className="w-3.5 h-3.5" />
                    <span>Edit Summary</span>
                  </button>
